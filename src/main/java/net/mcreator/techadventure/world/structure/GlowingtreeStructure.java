...
public class GlowingtreeStructure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 25000) {
						...
						for (int a = 0; a < count; a++) {
							...
							if (blockAt.getBlock() == GlowingdirtBlock.block)
								blockCriteria = true;
							if (!blockCriteria)
								continue;
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("tech_adventure", "v1"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("glowingtree"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("tech_adventure:glowingtree"), configuredFeature);
		}
	}
	@SubscribeEvent
	public static void addFeatureToBiomes(BiomeLoadingEvent event) {
		boolean biomeCriteria = false;
		if (new ResourceLocation("tech_adventure:the_glowing_lands").equals(event.getName()))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
